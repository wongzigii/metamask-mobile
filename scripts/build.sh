...
buildAndroidReleaseE2E(){
	prebuild_android
	cd android && ./gradlew assembleProdRelease app:assembleAndroidTest -PminSdkVersion=26 -DtestBuildType=release
}

buildAndroidQAE2E(){
	prebuild_android
	cd android && ./gradlew assembleQaRelease app:assembleAndroidTest -PminSdkVersion=26 -DtestBuildType=debug
}
